...
class Armadietto {
  constructor (options) {
    ...
    this._server = null;
    this._logger = Logger(options.logging || {});
    // we manually configure the logger on the storage
    // instance
    this._store.logger = this._logger;
    this.init();
  }
  ...
  async boot () {
    ...
    this._logger.info(`Starting remoteStorage: http://${this._options.http.host}:${this._options.http.port}`);
  }
  ...
  handle (req, res) {
    if (process.env.DEBUG) this._logger.debug(`${req.method} ${req.url} ${JSON.stringify(req.headers)}`);
    ...
    req.on('end', () => {
      ...
      this.dispatch(req, res)
        .catch(e => {
          this._logger.error('DISPATCH ERROR: ', e);
        });
    });

    req.on('error', err => this._logger.error(err.stack));
  }
  ...
  async dispatch (req, res) {
    ...
    if (match) {
      ...
      try {
        ...
        this._logger.error('Storage Error:', e);
      }
    }
