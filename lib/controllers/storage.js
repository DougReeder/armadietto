...
class Storage extends Controller {
  ...
  async checkToken (permission) {
    ...
    let permissions;
    try {
      console.log(`${this._username} at ${this.request?.headers['x-forwarded-for']} ${this.request?.socket?.address().address} presented ${this.getHost()} with token ${this._token}`);
      permissions = await this.server._store.permissions(this._username, this._token, this.getClientAddress(), this.getHost());
      if (!permissions) {
